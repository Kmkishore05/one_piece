...
import { Compass, Shield, Wind, Anchor, Sparkles } from 'lucide-react';
...
export const HeroSection: React.FC<HeroSectionProps> = ({ onExploreClick }) => {
  ...
  return (
    <section 
      id="hero"
      className="relative min-h-screen w-full bg-hero-ocean overflow-hidden flex flex-col justify-center pt-20"
      onMouseMove={handleMouseMove}
    >
      {/* Moving Sea & Parallax Sky Effects */}
      <div className="absolute inset-0 z-0 opacity-40 pointer-events-none">
        {/* Sky gradient clouds */}
        <div 
          className="absolute top-10 left-10 w-72 h-72 bg-[#3A506B]/30 rounded-full blur-3xl transition-transform duration-300"
          style={{ transform: `translate(${mouseOffset.x * 0.5}px, ${mouseOffset.y * 0.5}px)` }}
        />
        <div 
          className="absolute bottom-20 right-10 w-96 h-96 bg-[#E63946]/10 rounded-full blur-3xl transition-transform duration-300"
          style={{ transform: `translate(${-mouseOffset.x * 0.7}px, ${-mouseOffset.y * 0.7}px)` }}
        />
      </div>

      {/* Cloud Drifters */}
      <div className="absolute top-24 left-0 right-0 h-20 opacity-20 pointer-events-none overflow-hidden">
        <div className="absolute w-[200%] h-full bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-400 via-transparent to-transparent animate-cloud-slow" />
      </div>
      <div className="absolute top-48 left-0 right-0 h-16 opacity-10 pointer-events-none overflow-hidden">
        <div className="absolute w-[200%] h-full bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white via-transparent to-transparent animate-cloud-fast" />
      </div>

      {/* Main Container */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center w-full py-12">
        {/* Left Typography Block */}
        <div className="lg:col-span-7 space-y-6 text-center lg:text-left transition-transform duration-300"
          style={{ transform: `translate(${mouseOffset.x * 0.4}px, ${mouseOffset.y * 0.4}px)` }}>
          
          {/* Animated Straw Hat Badge */}
          <div className="inline-flex items-center space-x-2 bg-[#1C2541]/90 border border-[#FFD166]/40 px-3 py-1.5 rounded-full text-xs text-[#FFD166] tracking-widest uppercase font-mono shadow-[0_0_15px_rgba(255,209,102,0.15)] animate-pulse">
            <Sparkles className="w-3.5 h-3.5 text-[#FFD166]" />
            <span>Monkey D. Luffy Gear 5 Awakened World</span>
          </div>

          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black font-serif tracking-tight text-white uppercase leading-none">
            I'M GONNA BE<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E63946] via-[#FFD166] to-[#5BC0BE] text-glow-gold">
              THE KING OF THE PIRATES!
            </span>
          </h1>

          <p className="text-slate-300 text-sm sm:text-base max-w-xl mx-auto lg:mx-0 leading-relaxed font-sans">
            Inherited from Gol D. Roger, the legendary One Piece lies hidden at the end of the dangerous Grand Line. 
            Join Monkey D. Luffy and the Straw Hat Crew as they challenge Emperors, navigate gravity-defying seas, 
            and ignite the dawn of a new free world!
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-4">
            <button
              ...
              className="w-full sm:w-auto px-8 py-4 bg-[#E63946] text-white hover:bg-[#FFD166] hover:text-[#0B132B] font-bold uppercase text-xs tracking-widest rounded shadow-[0_4px_20px_rgba(230,57,70,0.4)] transition-all duration-300 flex items-center justify-center space-x-2 border border-[#E63946] group interactive-card"
            >
              <Compass className="w-4 h-4 group-hover:rotate-90 transition-transform duration-500" />
              <span>EXPLORE GRAND LINE</span>
            </button>

            <button
              onClick={() => handleClick('strawhats')}
              className="w-full sm:w-auto px-8 py-4 bg-[#1C2541]/90 text-[#FFD166] border border-[#FFD166] hover:bg-[#FFD166]/10 font-bold uppercase text-xs tracking-widest rounded transition-all duration-300 flex items-center justify-center space-x-2 group interactive-card"
            >
              <Anchor className="w-4 h-4 group-hover:translate-y-0.5 transition-transform" />
              <span>MEET STRAW HATS</span>
            </button>
          </div>

          {/* Quick Stats Grid */}
          <div className="grid grid-cols-3 gap-2 sm:gap-4 pt-8 border-t border-slate-800/60 max-w-md mx-auto lg:mx-0 text-center">
            <div className="p-2 bg-[#1C2541]/40 rounded">
              <div className="text-[#FFD166] font-bold text-lg">1100+</div>
              <div className="text-slate-400 text-[10px] uppercase font-mono">Anime Chapters</div>
            </div>
            <div className="p-2 bg-[#1C2541]/40 rounded">
              <div className="text-[#E63946] font-bold text-lg">3.5B+</div>
              <div className="text-slate-400 text-[10px] uppercase font-mono">Luffy Bounty</div>
            </div>
            <div className="p-2 bg-[#1C2541]/40 rounded">
              <div className="text-[#5BC0BE] font-bold text-lg">10</div>
              <div className="text-slate-400 text-[10px] uppercase font-mono">Crew Members</div>
            </div>
          </div>
        </div>

        {/* Right Interactive Thousand Sunny / Character Canvas Card */}
        <div className="lg:col-span-5 relative flex justify-center mt-6 lg:mt-0"
          ...
          {/* Outer framing glow circles */}
          <div className="absolute inset-0 bg-gradient-to-tr from-[#FFD166]/20 to-[#E63946]/10 rounded-3xl blur-3xl opacity-60 -z-10 animate-pulse" />

          {/* Premium Ship & Luffy Illustration Card */}
          <div className="w-full max-w-sm glass-panel rounded-2xl p-6 relative border border-[#FFD166]/30 shadow-2xl overflow-hidden animate-float-ship">
            {/* Top Comic banner item */}
            <div className="absolute top-0 right-0 bg-[#E63946] text-white text-[10px] uppercase font-bold px-3 py-1 font-mono tracking-widest rounded-bl shadow-md z-10">
              WANO TO EGGHEAD ARC
            </div>

            {/* Simulated Canvas Rendering */}
            <div className="h-64 w-full bg-[#0B132B] rounded-xl border border-slate-800 relative overflow-hidden flex flex-col justify-between p-4 bg-[radial-gradient(ellipse_at_bottom,_var(--tw-gradient-stops))] from-[#1C2541] via-[#0B132B] to-[#0B132B]">
              {/* Sun icon representing Thousand Sunny figurehead */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-20">
                <div className="w-40 h-40 rounded-full border-4 border-dashed border-[#FFD166] animate-spin" style={{ animationDuration: '40s' }} />
              </div>

              {/* Graphic Straw Hat Silhouette */}
...
<path id="_x23_F2F2F3" fill="#F2F2F3" d="M625.438,555.613c4.475,14.58-4.835,31.187-19.415,35.662  c-3.055,0.938-6.075,1.268-8.99,1.118c0.254,2.909,0.032,5.937-0.796,9.025c-3.952,14.731-20.216,24.628-34.946,20.676  c-14.731-3.952-22.35-20.257-18.398-34.987c1.554-5.791,4.777-10.406,9.007-13.639l-70.166-69.691  c9.278-7.371,18.043-15.353,26.235-23.887l68.54,68.075c3.08-4.344,7.577-7.729,13.309-9.488  C604.396,534.001,620.962,541.033,625.438,555.613z M128.747,479.889l-68.54,68.075c-3.08-4.344-7.577-7.729-13.309-9.488  c-14.58-4.475-31.146,2.558-35.621,17.138c-4.475,14.58,4.835,31.187,19.415,35.662c3.056,0.938,6.075,1.268,8.99,1.118  c-0.254,2.909-0.032,5.937,0.796,9.025c3.952,14.731,20.216,24.628,34.946,20.676c14.731-3.952,22.35-20.257,18.398-34.987  c-1.554-5.
.867,384.292,293.867,354.689z   M339.069,421.091c0-11.439-9.273-20.711-20.711-20.711c-11.439,0-20.711,9.273-20.711,20.711c0,11.439,9.273,20.711,20.711,20.711  C329.796,441.803,339.069,432.53,339.069,421.091z M450.051,354.689c0-29.604-23.998-53.602-53.602-53.602  c-29.604,0-53.602,23.998-53.602,53.602c0,29.604,23.998,53.602,53.602,53.602C426.052,408.291,450.051,384.292,450.051,354.689z   M523.614,137.641l53.608-53.245c6.929,8.83,18.851,13.671,29.911,10.703c14.731-3.952,22.35-20.256,18.398-34.987  c-3.576-13.331-15.981-20.448-29.223-18.757c2.162-13.173-4.508-25.823-17.702-29.873c-14.58-4.475-31.146,2.557-35.621,17.138  c-3.36,10.947,1.052,23.034,9.629,30.273l-52.963,52.605C508.215,119.661,516.22,128.398,523.614,137.641z M29.581,95.099  c11.06,2.967,22.982-1.873,29.911-10.703l53.608,53.245c7.395-9.242,15.4-17.98,23.964-26.143L84.1,58.893  c8.577-7.239,12.989-19.327,9.629-30.273c-4.475-14.58-21.041-21.613-35.621-17.138c-13.195,4.05-19.864,16.7-17.702,29.873  c-13.242-1.691-25.646,5.426-29.223,18.757C7.232,74.843,14.851,91.147,29.581,95.099z"></path>
              ...
              {/* Ocean Waves Simulator Layer */}
              <div className="absolute bottom-0 left-0 right-0 h-12 overflow-hidden opacity-50 flex items-end">
                <div className="w-[200%] h-6 bg-gradient-to-r from-[#5BC0BE] via-[#3A506B] to-[#5BC0BE] rounded-t-full animate-pulse opacity-80" />
              </div>

              {/* Card Badges */}
              <div className="flex justify-between items-start z-10">
                <div className="bg-[#0B132B]/80 px-2 py-1 rounded text-[10px] border border-slate-700 text-[#5BC0BE] font-mono">
                  LOG: NEW WORLD
                </div>
                <div className="bg-[#E63946]/20 px-2 py-1 rounded text-[10px] border border-[#E63946]/50 text-[#E63946] font-mono font-bold">
              ...
              <div className="z-10 text-center pb-2">
                <div className="text-xs uppercase font-mono tracking-widest text-slate-400">Flagship</div>
                <div className="text-xl font-bold font-serif text-[#FFD166] text-glow-gold tracking-wide">THOUSAND SUNNY</div>
                <div className="text-[10px] text-slate-400 mt-1 italic">"Coated & ready to plunge into the Deep Sea!"</div>
              </div>
            </div>

            {/* Quick Straw Hat Dialogue Box */}
            <div className="mt-4 p-3 bg-[#1C2541] border-l-4 border-[#FFD166] rounded-r text-xs space-y-1">
              <span className="font-bold text-[#E63946] block font-mono tracking-wider">💬 LUFFY'S DECLARATION:</span>
              <p className="text-slate-300 italic">
                "If you don't take risks, you can't create a future! Freedom is having the courage to sail past any storm!"
              </p>
            </div>

            {/* Compass Indicator Widget */}
            <div className="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between text-slate-400 text-[11px] font-mono">
              <span className="flex items-center"><Wind className="w-3.5 h-3.5 text-[#5BC0BE] mr-1" /> Koup de Burst: READY</span>
              <span className="flex items-center"><Shield className="w-3.5 h-3.5 text-[#FFD166] mr-1" /> Gaon Cannon: Active</span>
            </div>
          </div>
      ...
      {/* Sea Floor Wave Overlay Divider */}
      <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-none z-10">
        <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-full h-12 text-[#1C2541]" fill="currentColor">
